...
function toggleErrorMsgDetail(ele, msgId) {
    var toggle = document.getElementById(ele);
    var collapsedMsg = document.getElementById("msgc"+msgId);
    var expandedMsg = document.getElementById("msge"+msgId);
    var expand = expandedMsg.style.display == 'none';
    collapsedMsg.style.display = expand ? 'none' : '';
    expandedMsg.style.display = expand ? '' : 'none';
...
function showTestsForLine(ele, startLine, overTitle) {
    ...
    return overlib(parentTestDiv.innerHTML, SHADOW, SHADOWOPACITY, 80, CAPTION, aOverTitle, FIXX, xValue, STICKY);
}
